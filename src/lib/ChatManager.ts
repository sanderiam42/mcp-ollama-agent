...
export class ChatManager {
  ...
  private async handleToolCalls(toolCalls: any[]) {
    ...
    for (const toolCall of toolCalls) {
      ...
      const fixedArgs = this.fixToolArguments(toolCall.function.name, args, parameterMappings);
      ...
      if (result) {
        ...
        if (Array.isArray(resultContent) && 
            ...
            const toolInfo = this.toolManager.getToolParameterInfo(toolCall.function.name);
            ...
            if (newToolCalls.length > 0) {
                ...
                const hasNewToolCalls = newToolCalls.some(call => 
                    call.function.name !== currentToolName ||
                    JSON.stringify(call.function.arguments) !== JSON.stringify(toolCall.function.arguments)
                );
                
                if (hasNewToolCalls) {
                    await this.handleToolCalls(newToolCalls);
                } else {
                    console.log("Assistant: I apologize, but I'm having trouble with the correct parameter format. Let me try a different approach.");
                    return;
                }
            }
            return;
    ...
    if (newToolCalls.length > 0) {
        ...
        const hasNewTools = newToolCalls.some(call => !previousToolNames.has(call.function.name));
